...
function reduce(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case actions.STORE_ADD_ANONYMOUS_SUPPLY_REQUEST_RESET:
            ...
            return nextState || state;
        // ======================================================== Cancel supply
        // Resolve event to set cancel supply init request store data
        case actions.STORE_CANCEL_SUPPLY_REQUEST_INIT:
            nextState = {...state, cancel: requestInitValue()};
            return nextState || state;
        // Resolve event to set cancel supply failed request store data
        case actions.STORE_CANCEL_SUPPLY_REQUEST_FAILED:
            nextState = {...state, cancel: requestFailedValue(action.message)};
            return nextState || state;
        // Resolve event to set cancel supply succeeded request store data
        case actions.STORE_CANCEL_SUPPLY_REQUEST_SUCCEEDED:
            nextState = {...state, cancel: requestSucceededValue(action.message)};
            return nextState || state;
        // Resolve event to set cancel supply reset request store data
        case actions.STORE_CANCEL_SUPPLY_REQUEST_RESET:
