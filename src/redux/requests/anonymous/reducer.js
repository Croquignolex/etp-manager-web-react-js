...
function reduce(state = initialState, action) {
    ...
    switch (action.type) {
        ...
        case actions.STORE_NEXT_ANONYMOUS_REQUEST_RESET:
            ...
            return nextState || state;
        // ======================================================== Fleet supply
        // Resolve event to set add anonymous init request store data
        case actions.STORE_ADD_ANONYMOUS_REQUEST_INIT:
            nextState = {...state, add: requestInitValue()};
            return nextState || state;
        // Resolve event to set add anonymous failed request store data
        case actions.STORE_ADD_ANONYMOUS_REQUEST_FAILED:
            nextState = {...state, add: requestFailedValue(action.message)};
            return nextState || state;
        // Resolve event to set add anonymous succeeded request store data
        case actions.STORE_ADD_ANONYMOUS_REQUEST_SUCCEEDED:
            nextState = {...state, add: requestSucceededValue(action.message)};
            return nextState || state;
        // Resolve event to set add anonymous reset request store data
        case actions.STORE_ADD_ANONYMOUS_REQUEST_RESET:
